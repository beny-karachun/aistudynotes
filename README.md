...
- **Create notes with AI** — every folder has a *Create with AI* button (also in the right-click menus): upload a **PDF, images, or a text file**, optionally add instructions ("only chapter 2", "cloze for formulas"), and Gemini writes a full set of flashcards from it — **in the order the material appears in the document**, which is also the order you'll study them. You review the generated notes (untick any) before they're added, tagged `ai-generated`. Defaults to **Gemini 3.5 Flash — Thinking (high)** for the most careful reading; switchable per run. Files are sent directly to the Gemini API and stored nowhere else.
